...
public class TWAgentWorkingMemory {
    ...
    /**
     * Access to Scedule (TWEnvironment) so that we can retrieve the current
     * timestep of the simulation.
     */
    protected Schedule schedule;
    protected TWAgent me;
    protected final static int MAX_TIME = 10;
    protected final static float MEM_DECAY = 0.5f;

    protected ObjectGrid2D memoryGrid;

    protected TWAgentPercept target;
...
//        me.getEnvironment().getMemoryGrid().clear();  // THis is equivalent to only having sensed area in memory
        ...
        for (int i = 0; i < sensedObjects.size(); i++) {
            ...
            if (objects[objectXCoords.get(i)][objectYCoords.get(i)] == null) {
                memorySize++;
            }

            //Add the object to memory
            objects[objectXCoords.get(i)][objectYCoords.get(i)] = new TWAgentPercept(o, this.getSimulationTime(), 0);
...
//    }
    ...
    public void updateMemory(TWEntity[][] sensed, int xOffset, int yOffset) {
        for (int x = 0; x < sensed.length; x++) {
            for (int y = 0; y < sensed[x].length; y++) {
                objects[x + xOffset][y + yOffset] = new TWAgentPercept(sensed[x][y], this.getSimulationTime(), 0);
            }
        }
    }
    ...
    /**
     * removes all facts earlier than now - max memory time. TODO: Other facts
     * are remove probabilistically (exponential decay of memory)
     */
    public void decayMemory() {
        /*put some decay on other memory pieces (this will require complete iteration over memory though, so expensive.
        ...
        for (int x = 0; x < this.objects.length; x++) {
            for (int y = 0; y < this.objects[x].length; y++) {
                ...
                //current time - time at which object was seen cannot be greater than MAX_TIME
    ...
    public void removeAgentPercept(int i, int j) {
        // remove from memory
     ...
     *
     * @see TWAgentWorkingMemory#getNearbyObject(int, int, double,
     * java.lang.Class)
     */
    ...
    /**
     * Returns the nearest object that has been remembered recently where
     * recently is defined by a number of timesteps (threshold)
     *
     ...
     * objects in memory the method returns null. Note that specifying a
     * threshold of one will always return the most recently observed object.
     * Specifying a threshold of MAX_VALUE will always return the nearest
     * remembered tile.
     *
     * Also note that it is likely that nearby objects are also the most
     * recently observed
     *
     *
    ...
    public TWAgentPercept getCurrentLocPercept() {
        return objects[me.getX()][me.getY()];
    }

    public void removeCurrentLocPercept() {
        this.removeAgentPercept(me.getX(), me.getY());
    }
