...
public class TileworldMain {
    ...
    /**
     * Main method for restoring form checkpoints (see Tutorial 2 from Mason
     * docs)
     *
     * @param args
     */
    ...
    /**
     * Use the standard doLoop created in SimState, see Tutorial 2 for more
     * details. This method takes some standard argument format for non GUI
     * runs...
     *
     * Format: java tileworld.Tileworld \ [-help] [-checkpoint C] [-repeat R]
     * [-seed S] \ [-for F] [-until U] [-time T] [-docheckpoint D]
     *
     * -help Shows this message.
     *
     * -repeat R Long value > 0: Runs the job R times. The random seed for each
     * job is the provided -seed plus the job# (starting at 0). Default: runs
     * once only: job number is 0.
     *
     * -checkpoint C String: loads the simulation from file C for job# 0.
     * Further jobs are started new using -seed as normal. Default: starts a new
     * simulation rather than loading one.
     *
     * -until U Double value >= 0: the simulation must stop when the simulation
     * time U has been reached or exceeded. Default: don't stop.
     *
     * -for N Long value >= 0: the simulation must stop when N simulation steps
     * have transpired. Default: don't stop.
     *
     * -seed S Long value not 0: the random number generator seed. Default: the
     * system time in milliseconds.
     *
     * -time T Long value >= 0: print a timestamp every T simulation steps. If
     * 0, nothing is printed. Default: auto-chooses number of steps based on how
     * many appear to fit in one second of wall clock time. Rounds to one of 1,
     * 2, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, etc.
     *
     * -docheckpoint D Long value > 0: checkpoint every D simulation steps.
     * Default: never. Checkpoints files named
